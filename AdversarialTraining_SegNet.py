...
experiment = Experiment("m487mKQwNTqFF4z7aZRX3Xv19", project_name="AdversarialTraining_SegNet", log_env_gpu=True)
...
from Models import SegNet, Stanford_Adversarial
...
class SS_GAN(object):
    def __init__(self, image_shape, numClasses, pretrained_weights, palette, channels=None):
        ...
        # Build Generator
        self.generator = SegNet.build(self.input_shape, self.numOfClasses, pretrained_weights=pretrained_weights)
    ...
    def _define_composite(self, learning_rate):
        ...
        input2 = Input(shape=self.seg_shape)
        # segmentor output
    ...
    def train_Adversarial(self, dataset, testset, valset, epochs, n_batch, dataset_len):
        ...
        datagenerator = dataset.data_gen(should_augment=False, batch_size=n_batch)
        ...
        for epoch in range(0, epochs):
            ...
            for _ in tqdm(range(0, steps_per_epoch)):
                ...
                adv_loss = 0.5 * np.add(adv_loss_fake, adv_loss_real)
                dis_loss.append(adv_loss[0])
                dis_acc.append(adv_loss[1])
                ...
                # Plot The Progress
                print(
                    "{} [D loss: {}, acc.: {}%] [Composite G loss: {}]".format(epoch, adv_loss[0], 100 * adv_loss[1],
                                                                               g_loss[0]))
        ...
        df = pd.DataFrame(data={"g_loss": gen_loss, "g_acc": gen_acc, "d_loss": dis_loss, "d_acc":dis_acc,
                                    "steps": np.arange(steps).tolist()})
        df.to_csv("./output/training_data.csv", sep=',', index=False)
        df = pd.DataFrame(data={"val_steps": val_steps, "val_loss": val_loss, "val_acc": val_acc})
        df.to_csv("./output/validation_data.csv", sep=',', index=False)
    ...
    def _save_weights(self):
        ...
        """
        self.generator.save_weights('output' + '/AdvSegnet.h5')
    ...
    def _plot_results_and_save(self, g_loss, g_acc, d_loss, d_acc, steps, val_steps, val_loss, val_acc):
        ...
        plt.legend()
        plt.savefig('output/' + 'AdvSegnet_plot.png')
...
if __name__ == '__main__':
    ...
    Semantic_GAN = SS_GAN(image_shape=(256, 256),
                          numClasses=num_of_Classes,
                          pretrained_weights='SegNet_weights.h5',
                          palette=palette,
                          channels=[3, 3])
